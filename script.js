...
async function fetchVideos(searchQuery, pageToken) {
  try {
    ...
    const response = await fetch(
      `https://www.googleapis.com/youtube/v3/search?part=snippet&type=video&maxResults=${maxResults}&key=${apiKey}&q=${searchQuery}&pageToken=${pageToken}`
    );
    if (!response.ok) {
    ...
    videoCards.innerHTML = `
      <div class="container d-flex justify-content-center">
        <div class="alert alert-danger" role="alert">
                      404 Server Error! Please try again later.
        </div>
      </div>`;
  }
}
